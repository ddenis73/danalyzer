...
%%
    ...
    winSize = stageData.hdr.win;
    srate = stageData.hdr.srate;
    
    %Plot in Hours
    stageData.hdr.stageTime = stageData.hdr.stageTime ./3600;
    onT = hours(datetime(stageData.hdr.lOn, 'InputFormat', 'HH:mm:ss.SSS')...
        - datetime(stageData.hdr.recStart, 'InputFormat', 'HH:mm:ss.SSS'));
    if onT < 0
        onT = 24-abs(onT);
    end
    offT = hours(datetime(stageData.hdr.lOff, 'InputFormat', 'HH:mm:ss.SSS')...
        - datetime(stageData.hdr.recStart, 'InputFormat', 'HH:mm:ss.SSS'));
    ...
    if type == 1
        ...
        for i=1:size(stageRaster,1)
            sT = stageData.hdr.stageTime(find(~isnan(stageRaster(i,:))));
            
            for e = 1:size(sT,2)
                sTE = sT(e);
                
                if sTE>=(stageData.hdr.stageTime(min(find(stageData.stages<7)))) && sTE<=(stageData.hdr.stageTime(max(find(stageData.stages<7))))
        ...
        % xlabels = {datestr(stageData.recStart, 'HH:MM'), datestr(stageData.lOff,'HH:MM'), datestr(stageData.lOn, 'HH:MM')};
        ...
        plot(stageData.hdr.stageTime, connector, 'k','LineWidth',.2);
        ...
        set(gca, 'Xlim', stageData.hdr.stageTime([1, end]),'Ylim', [0, 9],'YTick', [1:8],'YTickLabel', [{'ANOM'; 'REM'; 'Stage4'; 'Stage3'; 'Stage2'; 'Stage1'; 'WAKE'; 'MT'; ''}]);
        set(gca,'TickDir','out');
        set(gca,'box','off')
    ...
    elseif type == 2
        ...
        for i=1:size(stageRaster,1)
            ...
            for e = 1:size(sT,2)
                ...
                if sTE>=(stageData.hdr.stageTime(min(find(stageData.stages<6)))) && sTE<=(stageData.hdr.stageTime(max(find(stageData.stages<6))))
        ...
        set(gca, 'Xlim', stageData.hdr.stageTime([1, end]),'Ylim', [0, 8],'YTick', [1:7],'YTickLabel', [{'ANOM'; 'REM'; 'SW'; 'Stage2'; 'Stage1'; 'Wake'; 'MT'; ''}]);
        set(gca,'TickDir','out');
        set(gca,'box','off')
