%6/24/14 ALTERNATE VERSION OF TOPOPLOT FUNCTION CREATED BY Michael Goldstein (MG)
    %to plot full nose and ears with 'headrad' 0.56 and 'plotrad'
    %0.65 specifiers (corresponding to plotting of inner 173 channels)
    %LOOK ON RIGHT SIDE WHILE SCROLLING TO SEE EDITS
...
function [handle,Zi,grid,Xi,Yi] = plot_topo(Values,loc_file,varargin)
...
if ~strcmpi(STYLE,'blank') % if draw interpolated scalp map
 if ~strcmpi(STYLE,'grid') %  not a rectangular channel grid
  ...
  try
%       [Xi,Yi,Zi] = griddata(inty,intx,intValues,yi',xi,'invdist'); %interpolate data                  %%%%% COMMENTED OUT BY MG 7/20/15
%       [Xi,Yi,ZiC] = griddata(inty,intx,intContourVals,yi',xi,'invdist'); % interpolate data           %%%%% COMMENTED OUT BY MG 7/20/15
      
      [Xi,Yi,Zi] = griddata(inty,intx,intValues,yi',xi,'v4'); % interpolate data                        %%%%% ADDED BY MG 7/20/15 - 'v4' is biharmonic spline
      [Xi,Yi,ZiC] = griddata(inty,intx,intContourVals,yi',xi,'v4'); % interpolate data                  %%%%% ADDED BY MG 7/20/15 - 'v4' is biharmonic spline
  catch,
...
if strcmp(SHADING,'interp')
%   rwidth = BLANKINGRINGWIDTH*1.3;             % width of blanking outer ring              %%%% MODIFIED BY MG BELOW: changed multiplication factor from 1.3 to 2.5
  rwidth = BLANKINGRINGWIDTH*2.5;             % width of blanking outer ring  
else
%   rwidth = BLANKINGRINGWIDTH;         % width of blanking outer ring                      %%%% MODIFIED BY MG BELOW: added multiplication factor of 2.5
  rwidth = BLANKINGRINGWIDTH*2.5;         % width of blanking outer ring  
end
...
axis equal;
% set(gca, 'xlim', [-0.525 0.525]); set(plotax, 'xlim', [-0.525 0.525]);                        %%%%% MODIFIED BY MG BELOW: changed +/- .525 to be flexible with "blanking ring" width
% set(gca, 'ylim', [-0.525 0.525]); set(plotax, 'ylim', [-0.525 0.525]);
set(gca, 'xlim', [min(ringx) max(ringx)]); set(plotax, 'xlim', [min(ringx) max(ringx)]); 
set(gca, 'ylim', [min(ringy) max(ringy)]); set(plotax, 'ylim', [min(ringy) max(ringy)]); 
